...
export function* encodeLiteral(
	...
	if (datatype === xsd.boolean) {
		...
		const byteLength = signed.encodingLength(i)
		yield* allocate(state, byteLength)
		signed.encode(i, state.buffer, state.offset)
		state.offset += byteLength
	} else if (datatype === xsd.nonNegativeInteger) {
		const i = parseInteger(value)
		integerValidators[xsd.nonNegativeInteger](i)
		const byteLength = unsigned.encodingLength(i)
		yield* allocate(state, byteLength)
		unsigned.encode(i, state.buffer, state.offset)
		state.offset += byteLength
