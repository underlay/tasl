(() => {
  ...
  var StateEffect = class {
    ...
    map(mapping2) {
      let mapped = this.type.map(this.value, mapping2);
    ...
    static mapEffects(effects, mapping2) {
      if (!effects.length)
        return effects;
      let result = [];
      for (let effect of effects) {
        let mapped = effect.map(mapping2);
        if (mapped)
          result.push(mapped);
      }
      return result;
  ...
  var StyleModule = class {
    constructor(spec, options) {
      ...
      function render(selectors, spec2, target) {
        ...
        for (let prop in spec2) {
          ...
          if (/&/.test(prop)) {
            render(selectors.map((s) => extend2 ? extend2(prop, s) : prop.replace(/&/g, s)), value, target);
    ...
    constructor(view2) {
      ...
      for (let type in handlers) {
        ...
        view2.contentDOM.addEventListener(type, (event) => {
          if (!eventBelongsToEditor(view2, event) || this.ignoreDuringComposition(event) || type == "keydown" && this.screenKeyEvent(view2, event))
            return;
  ...
  function applyDOMChange(view2, start, end, typeOver) {
    ...
    if (start > -1 && (bounds = view2.docView.domBoundsAround(start, end, 0))) {
      ...
      let head = iHead && iHead.node == domSel.focusNode && iHead.offset == domSel.focusOffset ? view2.state.selection.main.head : view2.docView.posFromDOM(domSel.focusNode, domSel.focusOffset);
      let anchor = iAnchor && iAnchor.node == domSel.anchorNode && iAnchor.offset == domSel.anchorOffset ? view2.state.selection.main.anchor : selectionCollapsed(domSel) ? head : view2.docView.posFromDOM(domSel.anchorNode, domSel.anchorOffset);
  ...
  var HoverPlugin = class {
    constructor(view2, source, field, setHover) {
      this.view = view2;
      this.source = source;
      this.field = field;
  ...
  function hoverTooltip(source, options = {}) {
    ...
    return [
      hoverState,
      ViewPlugin.define((view2) => new HoverPlugin(view2, source, hoverState, setHover)),
      tooltips()
    ];
  }
  ...
  var Parser = class {
    constructor(spec) {
      ...
      this.nested = (spec.nested || []).map(([name2, value, endToken, placeholder3]) => {
        return { name: name2, value, end: new TokenGroup(decodeArray(endToken), 0), placeholder: placeholder3 };
      });
  ...
  // ../taslx-lezer/grammar/taslx.js
  var spec_identifier = { __proto__: null, namespace: 8, map: 16 };
  var parser = Parser.deserialize({
    version: 13,
    states: "'zQYQPOOOeQPO'#C_OjQQO'#CcOOQO'#DS'#DSOOQO'#C|'#C|QoQPO'#C|QYQPOOOOQO'#Ca'#CaOtQSO,58yO!bQWO,58}OOQO,59h,59hOOQO-E6z-E6zOOQO1G.e1G.eO!oQPO'#CgO!tQPO1G.iOOQO'#Co'#CoOOQO'#Cq'#CqO#PQQO'#CsO#UQQO'#CuO#ZQPO'#CwOOQO'#DX'#DXOOQO'#DU'#DUO#`QQO1G.iO#eQQO'#C}O#pQPO'#DVO#uQPO,59RO#zQQO,59WO$PQWO,59_O$^QWO,59aO$kQQO'#DOO$vQPO'#DYO${QPO,59cO%QQWO7+$TO%[QPO'#CiOOQO,59i,59iO%dQQO,59qOOQO-E6{-E6{OOQO1G.m1G.mOOQO1G.r1G.rOOQO1G.y1G.yOOQO1G.{1G.{O%oQPO'#CyOOQO,59j,59jO%wQQO,59tOOQO-E6|-E6|OOQO1G.}1G.}O&SQPO<<GoOyQWO,59TP&_QQO'#C}O|QWO,59eP&gQQO'#DOO&oQPO1G.oOOQO1G/P1G/P",
!]%'Z!]!^%%O!_!`%%O!a!b$CY!b!c$>Y!c!}%%O#R#S%%O#T#o%%O#r#s%%O^%LqiXQdWqr%%Ost$:rtu%%Ouv%&qvw%%Owx%%Oxy%%Oyz%%Oz{%%O{|%%O|}%%O}!O%%O!O!P%,`!P!Q$Ae!Q!W%IO!W![%%O![!]%'Z!]!^%%O!_!`%%O!a!b$CY!b!c$>Y!c!}%%O#R#S%%O#T#o%%O#r#s%%O^%NghXQdWqr$Nvst$:rtu$Nvuv%!gvw$Nvwx$Nvxy$Nvyz$Nvz{$Nv{|$Nv|}$Nv}!O$Nv!O!P$Nv!P!Q$Ae!Q![%N`![!]$Nv!]!^$Nv!_!`$Nv!a!b$CY!b!c%#P!c!}$Nv#R#S$Nv#T#o$Nv#r#s$Nv^&!YhXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P&#t!P!Q$Ae!Q![&>q![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&#{kXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P$Jz!P!Q$Ae!Q!R&%p!R!S&%p!S!T&;S!T![&7n![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&%whXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P&'c!P!Q$Ae!Q![&7n![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&'jkXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P$Jz!P!Q$Ae!Q!R&)_!R!S&)_!S!T&4P!T![&0k![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&)fhXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P&+Q!P!Q$Ae!Q![&0k![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&+XkXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P$Jz!P!Q$Ae!Q!R$Jz!R!S$Jz!S!T&,|!T![$Jz![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&-TjXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P$Jz!P!Q$Ae!Q!V$Jz!V!W&.u!W![$Jz![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&.|iXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P$Jz!P!Q$Ae!Q!W$Jz!W![$Jz![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&0rhXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P&+Q!P!Q$Ae!Q![&2^![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&2ehXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P&+Q!P!Q$Ae!Q![$Jz![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&4WjXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P&+Q!P!Q$Ae!Q!V&0k!V!W&5x!W![&2^![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&6PiXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P&+Q!P!Q$Ae!Q!W&2^!W![$Jz![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&7uhXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P&'c!P!Q$Ae!Q![&9a![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&9hhXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P&'c!P!Q$Ae!Q![$Jz![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&;ZjXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P&'c!P!Q$Ae!Q!V&7n!V!W&<{!W![&9a![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&=SiXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P&'c!P!Q$Ae!Q!W&9a!W![$Jz![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&>xhXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P&#t!P!Q$Ae!Q![&@d![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&@khXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P&#t!P!Q$Ae!Q![$Jz![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&B^jXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P&#t!P!Q$Ae!Q!V&>q!V!W&DO!W![&@d![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz^&DViXQdWqr$Jzst$:rtu$Jzuv$Lmvw$Jzwx$Jzxy$Jzyz$Jzz{$Jz{|$Jz|}$Jz}!O$Jz!O!P&#t!P!Q$Ae!Q!W&@d!W![$Jz![!]$MV!]!^$Jz!_!`$Jz!a!b$CY!b!c%#P!c!}$Jz#R#S$Jz#T#o$Jz#r#s$Jz~&EyOl~~&FOOa~~&FTOo~~&FYO[~~&F_O_~",
    tokenizers: [0, 1, 2, 3],
  ...
  function mapKeys(object, map2) {
    const keys = getKeys(object);
    const result = Object.fromEntries(keys.map((key) => [key, map2(object[key], key)]));
  ...
  var reference = (value) => Object.freeze({ kind: "reference", value });
  var isReference = (type) => type.kind === "reference";
  var uri = () => Object.freeze({ kind: "uri" });
  var isUri = (type) => type.kind === "uri";
  var literal2 = (datatype) => Object.freeze({ kind: "literal", datatype });
  var isLiteral = (type) => type.kind === "literal";
  var product = (components) => Object.freeze({ kind: "product", components: Object.freeze(components) });
  var isProduct = (type) => type.kind === "product";
  var unit = () => product({});
  var isUnit = (type) => type.kind === "product" && getKeys(type.components).length === 0;
  var coproduct = (options) => Object.freeze({ kind: "coproduct", options: Object.freeze(options) });
  var isCoproduct = (type) => type.kind === "coproduct";

  // ../../node_modules/@underlay/apg/lib/schema/utils.js
  ...
  // ../../node_modules/@underlay/apg/lib/mapping/mapping.js
  var mapping_exports = {};
  ...
  // ../../node_modules/@underlay/apg/lib/mapping/apply.js
  var applyExpressions = (S, expressions, source) => expressions.reduce((type, expression) => apply(S, expression, type), source);
  function apply(S, expression, source) {
    if (expression.kind === "identifier") {
      return schema_exports.uri();
    } else if (expression.kind === "constant") {
      return schema_exports.literal(expression.datatype);
    } else if (expression.kind === "dereference") {
      if (source.kind === "reference" && source.value in S && source.value === expression.key) {
      ...
      return schema_exports.coproduct({ [key]: source });
    } else if (expression.kind === "tuple") {
      return schema_exports.product(mapKeys(expression.slots, (value) => applyExpressions(S, value, source)));
  ...
  // ../../node_modules/@underlay/apg/lib/mapping/utils.js
  ...
  function fold(M, S, T, type) {
    if (type.kind === "reference") {
      ...
      return product(mapKeys(type.components, (value) => fold(M, S, T, value)));
    } else if (type.kind === "coproduct") {
      return coproduct(mapKeys(type.options, (value) => fold(M, S, T, value)));
    } else {
  ...
  // ../../node_modules/@underlay/apg/lib/mapping/mapping.js
  var identifier = (value) => Object.freeze({ kind: "identifier", value });
  var constant = (value, datatype) => Object.freeze({ kind: "constant", value, datatype });
  var dereference = (key) => Object.freeze({ kind: "dereference", key });
  var projection = (key) => Object.freeze({ kind: "projection", key });
  var injection = (key) => Object.freeze({ kind: "injection", key });
  var tuple = (slots) => Object.freeze({ kind: "tuple", slots: Object.freeze(slots) });
  var match = (cases) => Object.freeze({ kind: "match", cases: Object.freeze(cases) });
  var map = (source, value) => Object.freeze({ kind: "map", source, value });
  var mapping = (maps) => Object.freeze(maps);

  // ../../node_modules/@underlay/apg/lib/mapping/delta.js
  var placeholder2 = new Product([]);

  // ../taslx-lezer/src/utils.ts
  ...
  function parseURI(state2, node) {
    ...
    if (prefix in state2.namespaces) {
      return state2.namespaces[prefix] + value.slice(index + 1);
    } else {
      const { from, to } = node;
  ...
  // lib/lint.js
  function lintView(view2) {
    const cursor = syntaxTree(view2.state).cursor();
    const slice = ({ from, to }) => view2.state.doc.sliceString(from, to);
    ...
    do {
      if (cursor.type.isError) {
        ...
        if (term !== null) {
          namespace = state2.slice(term);
        }
        const identifier2 = cursor.node.getChild("Prefix");
        if (identifier2 !== null) {
          const prefix = state2.slice(identifier2);
          if (prefix in state2.namespaces) {
            const { from, to } = identifier2;
            const message = `namespace ${prefix} has already been declared`;
            state2.diagnostics.push({ from, to, message, severity: "error" });
          } else {
            state2.namespaces[prefix] = namespace;
          }
        }
      } else if (cursor.type.name === "Expr") {
        const identifier2 = cursor.node.getChild("ExprName");
        const expressions = cursor.node.getChildren("Expression");
        const exprs = parseExprs(state2, expressions);
        if (identifier2 !== null) {
          const name2 = state2.slice(identifier2);
          if (name2 in state2.exprs) {
            const { from, to } = identifier2;
            const message = `expression ${name2} has already been declared`;
            state2.diagnostics.push({ from, to, message, severity: "error" });
          } else {
            state2.exprs[name2] = exprs;
          }
        }
      } else if (cursor.type.name === "Map") {
        const terms = cursor.node.getChildren("Term");
        if (terms.length > 0) {
          const key = getURI(state2, terms[0]);
          if (key !== null) {
            if (key in state2.mapping) {
              const { from, to } = terms[0];
              const message = `map ${key} has already been declared`;
              state2.diagnostics.push({ from, to, message, severity: "error" });
            } else {
              const expressions = cursor.node.getChildren("Expression");
              const source = terms.length > 1 ? getURI(state2, terms[1]) : key;
              state2.mapping[key] = mapping_exports.map(source, parseExprs(state2, expressions));
            }
          }
        }
      }
      reportChildErrors(state2.diagnostics, cursor);
    } while (cursor.nextSibling());
    const namespaces = Object.entries(state2.namespaces).filter(([_, base2]) => base2 !== null);
    const sorted = state2.diagnostics.sort(({ from: a, to: c }, { from: b, to: d }) => a < b ? -1 : b < a ? 1 : c < d ? -1 : d < c ? 1 : 0);
    return {
      errorCount: sorted.length,
      mapping: state2.mapping,
      namespaces: { ...defaultNamespaces, ...Object.fromEntries(namespaces) },
      diagnostics: sorted
    };
  }
  function getURI(state2, node) {
    try {
  ...
  var parseExprs = (state2, nodes) => nodes.reduce((prev, node) => prev.concat(parseExpr(state2, node)), []);
  function parseExpr(state2, node) {
    if (node.name === "Variable") {
      const value = state2.slice(node);
      if (value in state2.exprs) {
        return state2.exprs[value];
      } else {
        const { from, to } = node;
      ...
      const key = getURI(state2, term);
      return [mapping_exports.dereference(key)];
      ...
      const key = getURI(state2, term);
      return [mapping_exports.projection(key)];
      ...
      const key = getURI(state2, term);
      return [mapping_exports.injection(key)];
      ...
      const key = getURI(state2, term);
      return [mapping_exports.identifier(key)];
    } else if (node.name === "Constant") {
      const string2 = node.getChild("String");
      const value = string2 === null ? '""' : JSON.parse(state2.slice(string2));
      const term = node.getChild("Term");
      if (term === null) {
        return [];
      }
      const key = getURI(state2, term);
      return [mapping_exports.constant(value, key)];
      ...
      for (const slot of node.getChildren("Slot")) {
        ...
        slots[key] = parseExprs(state2, expressions);
      }
      return [mapping_exports.tuple(slots)];
      ...
      for (const CASE of node.getChildren("Case")) {
        ...
        cases[key] = parseExprs(state2, expressions);
      }
      return [mapping_exports.match(cases)];
    } else {
  ...
  var LintDelay = 500;
  var setMappingEffect = StateEffect.define();
  var MappingState = StateField.define({
    create() {
      return { errorCount: 0, mapping: {}, namespaces: defaultNamespaces };
    },
    update(value, tr) {
      return tr.effects.reduce((value2, effect) => effect.is(setMappingEffect) ? effect.value : value2, value);
    }
  });
  var linter = [
    MappingState,
  ...
  function addMappingToBranch(branch, mapping2) {
    if (!branch.length)
      return branch;
    let length = branch.length, selections = none3;
    while (length) {
      let event = mapEvent(branch[length - 1], mapping2, selections);
      if (event.changes && !event.changes.empty || event.effects.length) {
        let result = branch.slice(0, length);
        result[length - 1] = event;
        return result;
      } else {
        mapping2 = event.mapped;
        length--;
  ...
  function mapEvent(event, mapping2, extraSelections) {
    let selections = conc(event.selectionsAfter.length ? event.selectionsAfter.map((s) => s.map(mapping2)) : none3, extraSelections);
    if (!event.changes)
      return HistEvent.selection(selections);
    let mappedChanges = event.changes.map(mapping2), before = mapping2.mapDesc(event.changes, true);
    let fullMapping = event.mapped ? event.mapped.composeDesc(before) : before;
    return new HistEvent(mappedChanges, StateEffect.mapEffects(event.effects, mapping2), fullMapping, event.startSelection.map(before), selections);
  }
  var HistoryState = class {
    ...
    addMapping(mapping2) {
      return new HistoryState(addMappingToBranch(this.done, mapping2), addMappingToBranch(this.undone, mapping2), this.prevTime, this.prevUserEvent);
    }
  ...
  function mapRange(range, mapping2) {
    let from = mapping2.mapPos(range.from, 1), to = mapping2.mapPos(range.to, -1);
  ...
  function selectedLines(view2) {
    ...
    for (let { head } of view2.state.selection.ranges) {
      if (lines.some((l) => l.from <= head && l.to >= head))
        continue;
      lines.push(view2.visualLineAt(head));
    }
    return lines;
  }
  ...
  var deleteByChar = (view2, forward, codePoint) => deleteBy(view2, (pos) => {
    ...
    if (codePoint) {
      ...
      target = forward ? Math.min(state2.doc.length, pos + size) : Math.max(0, pos - size);
    } else {
      target = findClusterBreak(line.text, pos - line.from, forward) + line.from;
    }
    if (target == pos && line.number != (forward ? state2.doc.lines : 1))
      target += forward ? 1 : -1;
    return target;
  });
  ...
  var closeBracketEffect = StateEffect.define({
    map(value, mapping2) {
      let mapped = mapping2.mapPos(value, -1, MapMode.TrackAfter);
      return mapped == null ? void 0 : mapped;
    }
  });
  var skipBracketEffect = StateEffect.define({
    map(value, mapping2) {
      return mapping2.mapPos(value);
    }
  });
  ...
  function handleSame(state2, token, allowTriple) {
    let dont = null, changes = state2.changeByRange((range) => {
      ...
      if (next == token) {
        ...
        if (prev != token && state2.charCategorizer(pos)(prev) != CharCategory.Word)
          return {
            changes: { insert: token + token, from: pos },
            effects: closeBracketEffect.of(pos + token.length),
            range: EditorSelection.cursor(pos + token.length)
...
<button class=${themeClass("button")} type=submit>${view2.state.phrase("go")}</button>`;
  ...
  function applyCompletion(view2, option) {
    let apply2 = option.completion.apply || option.completion.label;
    let result = option.source;
    if (typeof apply2 == "string") {
      view2.dispatch({
        changes: { from: result.from, to: result.to, insert: apply2 },
        selection: { anchor: result.from + apply2.length }
      });
    } else {
      apply2(view2, option.completion, result.from, result.to);
    }
  }
  ...
  var ActiveSource = class {
    constructor(source, state2, explicit) {
      this.source = source;
      this.state = state2;
      this.explicit = explicit;
    }
  ...
  var ActiveResult = class extends ActiveSource {
    ...
    map(mapping2) {
      return new ActiveResult(this.source, this.explicit, this.result, mapping2.mapPos(this.from), mapping2.mapPos(this.to, 1), this.span);
    }
  };
  ...
  var setActiveEffect = StateEffect.define({
    map(sources, mapping2) {
      return sources.map((s) => s.hasResult() && !mapping2.empty ? s.map(mapping2) : s);
    }
  });
  ...
  function changeBlockComment(option, ranges, state2) {
  ...
  function changeLineComment(option, ranges, state2) {
    ...
    if (option != 2 && lineRanges.some((c) => !c.commented)) {
      let changes = [];
      ...
      return { changes };
    } else if (option != 1 && lineRanges.some((c) => c.commented)) {
      let changes = [];
      for (let i = 0, lineRange; i < ranges.length; i++)
        if ((lineRange = lineRanges[i]).commented) {
          let token = tokens[i];
